...
export default function FeedCard({ post, isLoading }: FeedCardProps) {
  ...
  return (
    <div key={post.id} className="flex w-4/5 flex-col">
      <Link
        href={`/userProfile/${post.user.id}`}
        className="flex h-[70px] w-[274px] items-center justify-center gap-[13px] text-white"
      >
        <Avatar
